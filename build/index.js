...
class Search {
  ...
  async getResults() {
    try {
      // UPDATED:
      const results = await axios__WEBPACK_IMPORTED_MODULE_0__["default"].get(`${universityData.root_url}/wp-json/university/v1/search?term=${this.searchField.value}`);
      console.log(results);
      this.resultsDiv.innerHTML = `
        <div class="row">
          <div class="one-third">
            <h2 class='search-overlay__section-title'>General Information</h2>
            ${results.data.generalInfo.length ? `<ul class='link-list min-list'>` + results.data.generalInfo.map(item => `
                        <li><a href='${item.permalink}'>${item.title}</a> ${item.postType == "post" ? "by " + item.authorName : ""}</li>
                      `).join("") + `</ul>` : "<p>Nothing to show here..</p>"}
          </div>
          <div class="one-third">
            <h2 class='search-overlay__section-title'>Programs</h2>
            ${results.data.programs.length ? `<ul class='link-list min-list'>` + results.data.programs.map(item => `
                        <li><a href='${item.permalink}'>${item.title}</a></li>
                      `).join("") + `</ul>` : `<p>Nothing to show here..<a href='${universityData.root_url}/programs'>View all</a></p>`}
            <h2 class='search-overlay__section-title'>Professors</h2>
            ${results.data.professors.length ? `<ul class='professor-cards'>` + results.data.professors.map(item => `
...
" alt="">
          ...
          <div class="one-third">
            ...
            ${results.data.events.length ? `<ul class='link-list min-list'>` + results.data.events.map(item => `
    ...
    <div class="event-summary__content">
        ...
        ${item.description}
            <a href="${item.permalink}" class="nu gray">Read more</a>
                      ...
                      `).join("") + `</ul>` : `<p>Nothing to show here..<a href='${universityData.root_url}/events'>View all</a></p>`}
          </div>
        </div>
      `;
    } catch (error) {
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = self["webpackChunkuniversitytheme"] = self["webpackChunkuniversitytheme"] || [];
